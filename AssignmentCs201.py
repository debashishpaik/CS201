class NFA:
    ...
    def __init__(self):
        ...
        self.M = [[0 for i in range(len(self.Sigma))] for j in range(len(self.Q))]
    ...
    def states(self):        #function for storing the states of the NFA
        ...
        print(self.Q)        # displaying the list of display the states of the NFA given a input by the user
    ...
    def alpahabet(self):     #function for storing the alphabets(symbols) of the NFA
        ...
        print(self.Sigma)  #displaying the alphabet list of the NFA taken as input by the user
    ...
    def matrix_store(self, i, j):                                   #method for defining the delta function of the NFA
        ...
        for t in range(0,n):
            list.append(input("give the " +str(t+1)+ " th state : "))  #appending display the states to which Q[i] can transit after getting the symbol Sigma[j]

        self.M[i][j]=list                                           # delta( Q[i] , Sigma[j] ) = list



    def transition_func_value_storing(self):
        self.M = [[0 for i in range(len(self.Sigma))] for j in range(len(self.Q))]  #matrix for storing the corresponding set of states at which a certain state of the NFA will transit after getting a particular symbol contained in the alphabet set
    ...
    def display(self):
        for i in range(0, len(self.Q)):
            for j in range(0, len(self.Sigma)):
                print(" The state "+ str(self.Q[i])+" will transit to "+str(self.M[i][j])+" states after being acted upon by the symbol  "+ str(self.Sigma[j]))
    ...
    def delta_modified(self, lst, x):
        ...
        for i in range(0, len(lst)):
            list1=list(set(self.M[self.Q.index(lst[i])][self.Sigma.index(x)])|set(list1)) #union of display the set of states to which the elements in the list 'lst' transited after being acted upon by x

        #print(" the union of display the states to which " + str(lst)+ " transited after getting the symbol "+ str(x)+ " is :")
        #print(list1)
        return list1

    def closure_delta_modified(self, l, y): #method for evaluating the union of epsilon closures of display the elements in the list returned by the method 'delts_modified'
        ...
        if lst1!=[]:                                                                   #if lst1 is not empty
            for i in range(0, len(lst1)):
                ...
                str_lst = list(set(str_lst) | set(self.initial_state_DFA(list(set(self.M[ind][0]) | set([lst1[i]])))))  #union of the epsilon closure of display the states in the list 'l'
    ...
    def dfa_states_store(self, i=0, lt=[], M_str=[]):
        ...
        M_str.append([self.closure_delta_modified(lt[i],x) for x in self.A1])  # here A1 is the alphabet set consisting of display the alphabets except epsilon
        length=len(lt)
        for x in M_str[i]:
            if x != []:
        ...
        if length1==length and i+1==length1:         #if no new state is added to lst and no element in lst is left to be processed
            return [lt, M_str]                      # lst is the list of display states in the required DFA
